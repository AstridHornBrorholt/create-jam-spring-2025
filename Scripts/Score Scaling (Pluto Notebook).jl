...
# ╔═╡ 6f8b5b9c-42dc-11f1-303f-c90703cab883
begin 
	using Plots
	using Plotly
	using PlutoUI
...
# ╔═╡ 224ad613-8a25-4474-8bdc-815b975b9256


# ╔═╡ e684ec6e-4cba-4bf3-af0e-986f03443440
begin
	plotly()
	plot = Plots.plot
...
# ╔═╡ 4e87c0a4-8090-4c8e-8478-f164c6618667
win_level = 20

# ╔═╡ 637a68a2-ad4a-4ed6-a5f9-eb3ffecd696d
function standard(level)
	a = 250
	b = 200
	c = 33.335
	d = 2
	...
	if level < 8
		return a*level + b
	elseif level <= win_level
		# Increase goal quadratically from now on
		return c*(level - 8)^d + a*(level) + b
	else
		# Add an exponential component. 
		return e + f*g^(level - win_level) + c*(level - 8)^d + a*(level) + b
	end
end

# ╔═╡ 200462e2-a1a9-4560-9f62-3214c678437e
plot(level -> 20*(level - 8)^2, xlim=(8, 20))

# ╔═╡ b6d92684-3052-48c8-b26d-e4bf4cb0a82e
standard(20)

# ╔═╡ e9573c6a-4765-4a8a-8f17-d55874267d47
function easy(level)
	a = 150
	b = 150
	c = 12.85
...
# ╔═╡ 7e07291d-6022-4dc0-979a-5e6c8951f5a9
easy(20)

# ╔═╡ dc413947-7175-4363-8705-1ceae8ad38f0
function challenge(level)
	...
	elseif level <= win_level
		# Increase goal quadratically from now on
		return c*(level - 5)^d + a*(level) + b
	else
		# Add an exponential component. 
		return e + f*g^(level - win_level) + c*(level - 5)^d + a*(level) + b
	end
end

# ╔═╡ e090e912-638d-4b0f-849f-a64bb1edb17d
challenge(20)

# ╔═╡ 1bc4a1cf-ed31-40e7-a2f9-485e591553a7
@bind x_max NumberField(20:100, default=30)

# ╔═╡ b6fc99d8-280e-42de-8750-1bad278498c5
scatter([(x, standard(x)) for x in 0:x_max],
		color=:white,
		label=nothing)

# ╔═╡ 351ea900-649a-4129-8b0a-6f2b079f83bf
scatter([(x, easy(x)) for x in 0:x_max],
		color=:blue,
		label=nothing)

# ╔═╡ 377fb18f-c591-46ad-b8a2-1ba9f0bb4e48
scatter([(x, challenge(x)) for x in 0:x_max],
		color="#ff0000ff",
		label=nothing)

# ╔═╡ 87d8cd78-e9c8-4ef8-a439-af1b787b7fa0
function unreasonable(level)
	a = 1200
	b = 1000
	c = 288.8889
	d = 3
...
# ╔═╡ c29905e8-3829-4bd4-9c23-163e3ee0f5bc
unreasonable(20)

# ╔═╡ 6d684974-3cf5-4944-82f1-8e5c70752dc0
scatter([(x, unreasonable(x)) for x in 0:x_max],
		color="#ff86ff",
		label=nothing)
...
manifest_format = "2.0"
project_hash = "bd69da1b64fd12223848340376724d3edab3ef03"
